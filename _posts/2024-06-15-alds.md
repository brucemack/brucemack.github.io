...
This project wouldn't be possible without access to the actual schematics of the 
IBM 1620. *A huge thanks goes to Dave Babcock* who has loaned me his set of original 
model 1F system diagrams, complete with the cool 11x17 binders. [Dave is a 1620 developer himself](https://github.com/IBM-1620), 
having worked on the amazing IBM 1620 Jr. project, the Cadetwriter, and an SPS1620 cross-assembler.

I had the pleasure of working as a VLSI designer on a mainframe system development team 
at IBM Poughkeepsie in the early 
1990's. It was obvious at that time - and is still today - that electronics designers
are only as good as the software tools they have access to. Modern-day tools like Cadence, Synopsys, KiCad, Altium, 
and ANSYS are the bread and butter of electronics design.

As I looked at the original 1620 schematics, the first thing I wondered is how they were created. Some history 
of these diagrams is interesting (to me, anyhow).

## Engineering Design Automation in 1959

A computer of even modest complexity requires the interaction of 
...
computer, an engineering team relies heavily on *the previous-generation* computer to enable their task. So-called engineering design automation tools (EDA) are a way of life 
...
> and up-dating the information necessary ... Automation of design eliminates the slow
...
of design automation in their detailed chapter on the advent of transistor technology:
...
The primary EDA technology that both of these books are referencing was called the "Design
Mechanization System."  This revolutionary system produced the Automated 
Logic Diagrams (ALDs). It's ironic that the elimination of paperwork is referenced in 
both of the quotes above, yet the only representation of the 1620 circuit we have left 
is in giant binders containing 400 nearly pages of paper ALD diagrams!
...
misses the innovations that enabled the 1620 design (no to mention the 1401, 7090, and S/360). I used 
many of the tools 
...
Where things get interesting from a software simulation standpoint is in the section of the _System Diagram_ books that contains the complete ALD of the machine that the binders are associated with. Here's a typical example of an ALD page:

![ALD](/assets/images/IMG_0990.jpg).
...
key punching. The article highlights the fact that these forms were structured such that "no knowledge
of circuit logic or convention is required" to do this data entry job. The claim is made that a 
...
* The Master Tape was then used to print an automated representation of the design, which was returned to 
the engineering team for review and modification.
* A rigorous change-control workflow was used to manage ECs. This involved another set of forms and another 
set of record added to the Master Tape.  Through this method, every step in a design's evolution could be 
tracked.

From the perspective of the reverse-engineering project, one of the most interesting parts of this 
article is the section entitled, simply: _Print Program_. 
...
* An IBM 717 printer was used, which was part of of the 705 accounting computer system. According to one document
I found, the 717 was a variant of the 716. This printer supported a line width of 120 characters using 120 printing wheels.  Each wheel contained 48 characters: the 26 upper-case letters, the numbers 0-9, and twelve "special characters."  From what I can tell from Dave's ALDs, those special characters were the 
period, plus, minus, pound/hash, and the characters needed to 
produce the distinctive box/wiring diagrams (horizontal line, vertical line, top left corner, 
top right corner, bottom left corner, bottom right corner, cross, and diamond). In fact, the article 
mentions that the 717 type wheel was modified specifically to add these special diagraming symbols. This finally explains the 
use of the odd "bottom right" character in many of the 1620 page names and signal names.  For example, 
page 01.10.12.1: TIMING SAMPLE GENERATION A┘B. That strange mark was obviously being displayed in place of the slash
character which wasn't available on the modified 717 printer.  (NOTE: That bottom-right symbol is Unicode x2518 if you need to type it.)
* Blocks are 6x7 characters. The location of each block on the page, and the location of the characters
within the block is completely fixed.
* The total page is 120x186 characters - an aspect ratio chosen to fit on 11x17 inch "B" size ledger paper. 
* The wiring lines on the diagrams were *automatically routed* by the software, based on the engineer's
input about starting and ending locations. This is very impressive.
* The paper describes the creation of an in-memory representation of the entire page (22,320 characters)
during the printing process. The creation of this "frame buffer" of sorts required an average of 45 seconds
of computer time. To put this in perspective, the base 1620 core memory could only store 20,000 decimal digits. This gives a sense of how the early EDA tools were pushing the boundaries of technology.
...
The letters around the perimeter of the block correspond to the pin letters of the SMS card.
...
of these diagrams. Automated scanning is critical to success.
...
rules because they will apply to any modern scanning system I create.  60 years later, the same rules apply:
...
The background in this post provides one piece of context that makes the scanning 
...
After reading about the history of EDA at IBM, I am very confident that the development team at the 
